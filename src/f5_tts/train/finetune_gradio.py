...
# Add this after exp_name declaration
...
def start_training(
    ...
    if exp_name == "F5TTS_Small":
        # Force finetune to False for F5TTS_Small
        finetune = False
        # No pretrained checkpoint needed
        file_checkpoint_train = ""
    ...
    if finetune and exp_name != "F5TTS_Small":
        cmd += " --finetune"
...
```""")
            ...
            with gr.Row():
                exp_name = gr.Radio(
                    label="Model",
                    choices=["F5TTS_Base", "E2TTS_Base", "F5TTS_Small"],
                    value="F5TTS_Small"  # Set F5TTS_Small as default
                )
            ...
            def update_finetune_option(model_name):
                if model_name == "F5TTS_Small":
